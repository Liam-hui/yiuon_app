...
import { Text,Keyboard, View, StyleSheet,ImageBackground,TouchableOpacity,TouchableHighlight,Image,SafeAreaView,Dimensions} from 'react-native';
...
export default function Chatroom({navigation,route}) {
    ...
    const renderHeaderRight = () => (
        <TouchableOpacity
            onPress={() => {navigation.navigate('chat-setting', {group:groupRef.current, title: groupRef.current.title, other:other, onGoBack:(updated_group)=> {group = updated_group;}, changeTitle:(title)=>navigation.setParams({title:title}) 
        }) }}
        >
    ...
    const renderBubble = (props) => { 
        let bubbleWrapperStyle,bubbleStyle,bubbleCorner;
        ...
        if(props.currentMessage.type!='audio') bubbleCorner = styles.bubbleCorner; else bubbleCorner = styles.bubbleCornerAudio;
    ...
    <SafeAreaView>
        ...
        >
            ...
            {showPickImage? (
                <>
                <PickImage
                    addPhoto={(url)=>{setImageUrl(url);setShowFileChosen(true);}}
                    mode={uploadMode}
                    turnOff={()=>setShowPickImage(false)}
                />
                </>
                ):(null)
            }
